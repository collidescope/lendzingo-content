...
export function RelatedArticleCardShell({ image, category, title, excerpt, href }: RelatedArticleCardProps) {
  return (
    <article className="bg-white border border-gray-200 rounded-xl overflow-hidden hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 flex flex-col">
      ...
      <div className="p-5 flex flex-col flex-1">
        ...
        <h3 className="font-serif text-base font-bold text-lendzingo-ink leading-snug mb-2">{title}</h3>
        <p className="font-sans text-sm text-lendzingo-muted leading-relaxed mb-4 flex-1">{excerpt}</p>
        <Link
          href={href}
...
export default function RelatedArticleCard(props: RelatedArticleCardProps) {
  ...
  const decoratedHref = (() => {
    ...
    return params.length ? `${props.href}?${params.join('&')}` : props.href
  })()

  return <RelatedArticleCardShell {...props} href={decoratedHref} />
}
